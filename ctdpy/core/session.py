...
class Session:
    ...
    Example usage:
        ...
        # Datasets are stored in a list of 2 (0: data, 1: metadata).
        # For this example we only have data, no metadata.
    ...
    def __init__(self, filepaths=None, reader=None, export_path=None):
        ...
        Create reader instance and set path to export path.

        Args:
            filepaths (iterable): A sequence of files that will be
        ...
        if filepaths and reader:
            ...
            self.readers = self.create_reader_instances(
                filepaths=filepaths, reader=reader
            )
    ...
    def read(self, add_merged_data=False, add_low_resolution_data=False):
        ...
        Args:
            add_merged_data (bool): WILL PROBABLY BE MOVED AND USED ELSEWHERE.
                                    False or True. Merge metadata and data
                                    into template.
            add_low_resolution_data (bool): WILL PROBABLY BE MOVED AND USED
                                            ELSEWHERE. Include extra
                                            pd.DataFrame for low resolution data
        """
        if add_merged_data:
            raise FutureWarning(
                'The functionality of ctdpy.core.session.Session.read('
                'add_merged_data=True) will be moved elsewhere in the future.')
    ...
    def _read_datasets(self, add_merged_data, add_low_resolution_data):
        ...
        for dataset in self.readers:
            ...
            if add_merged_data and add_low_resolution_data:
                self.readers[dataset]['reader'].merge_data(
                    data, resolution='lores_data'
                )
    ...
    def _get_filenames_matched(filenames, file_type):
        ...
        if file_type.get('file_patterns'):
            ...
            filenames_matched = get_file_list_based_on_suffix(
                filenames, file_type['file_suffix']
            )
        else:
            raise ImportWarning(
                'No file_pattern nor suffix is readable in reader.file_types')
    ...
    def get_data_in_template(self, datasets, template=None, resolution='lores'):
        ...
        for fid in datasets:
            if resolution == 'lores':
                ...
                data = profile.extract_lores_data(
                    key_depth='DEPH [m]', discrete_depths=self.settings.depths
                )
            else:
            ...
            meta = meta_handler.get_meta_dict(
                ...
                keys=self.settings.templates[template]['template'].get(
                    'meta_parameters'),
                identifier='//METADATA',
                separator=';',
            )
    ...
    def get_writer_file_name(self, writer):
        ...
        # Fixme change 'filename' to 'filename_prefix'?
        #  perhaps not prefix in all cases?
        return self.settings.writers[writer]['writer'].get('filename')

    def load_template_handler(self, template):
        """Return Template instance."""
        template_instance = self.settings.templates[template].get(
            'template_handler')
    ...
    def update_metadata(datasets=None, metadata=None, overwrite=False):
        """Update the given datasets with information in metadata.
    ...
    def update_settings_attributes(self, **kwargs):
        """Update setting attributes.

        Settings with specified reader kwargs at a
        higher level within the settings dictionary tree.

        Args:
            kwargs: eg. reader settings
        """
        # FIXME Do we need to add attributes of specified reader to a
        #  higer level within the self.settings structure?
        self.settings.set_attributes(self.settings, **kwargs)
